...
public class HoaDonDao {
    ...
    public List<TopNV> getTop(){
        String sqlTop = "SELECT maNV, count(maNV) as soLuong FROM HoaDon GROUP BY maNV ORDER BY soLuong DESC LIMIT 20";
        List<TopNV> list = new ArrayList<TopNV>();
        NhanVienDAO nhanVienDAO = new NhanVienDAO(context);
        Cursor c = db.rawQuery(sqlTop,null);
        while (c.moveToNext()){
            TopNV topNV = new TopNV();
            NhanVien nhanVien  = nhanVienDAO.getID(c.getString(0));
            topNV.setTenNV(nhanVien.getTenNV());
